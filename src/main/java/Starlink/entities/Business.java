...
public class Business extends User{
    ...
    public boolean updateAccount(int userID, String username, String password, String email, String businessID, String name, String address) throws Exception
    {
        try{
            ...
            results = SQLHelper.selectStatement(String.format("select userID from business where businessid = '%s'", businessID));
            while(results.next()){
                if(results.getInt("userID") != userID) //Make sure we're not checking against the account being updated
                    throw new Exception("Another account already uses this business ID!");
            }
